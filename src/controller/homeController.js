            ...
            dbpool.query("SELECT account.id,account.typegame,account.account,account.password,status.clientid,status.clientname FROM `account` LEFT JOIN `status` ON account.id = status.id;",
                function(err, account) {
                    if (err) {
                        ...
                        return res.render('index', { DataType: JSON.stringify(namegame), DataAccount: JSON.stringify(account) })
                    }
                })
    ...
    dbpool.query("select * from namegame where typegame = ?  ", typegame, function(err, insert) {
        if (err) {
            ...
            if (insert == "") {
                ...
                return res.send(`<script>window.alert("Thêm Thành công"); window.location.href = "/"; </script>`)
            } else {
                console.log("Có", insert);
                return res.send(`<script>window.alert("Thêm Thất Bại Vì đã trùng loại game"); window.location.href = "/"; </script>`)
            }
        }
    })
}
...
let addAccount = async(req, res) => {
    ...
    if (typegame == "Chọn loại game") return res.redirect('/');
                        ...
                        dbpool.query(`SELECT * FROM account ORDER BY id DESC LIMIT 1`, function(err, countid) {
                            if (err) {
                                ...
                                if (account == "") {
                                    ...
                                    return res.send(`<script>window.alert("Đã thêm thành công"); window.location.href = "/"; </script>`)
                                } else {
                                    return res.send(`<script>window.alert("Thêm thất bại vì đã trùng tài khoản"); window.location.href = "/"; </script>`)
                                }
                            }
                        })
                    }
...
let editAccount = async(req, res) => {
    ...
    if (typegamenew == "Chọn loại game") return res.redirect('/');
            ...
            dbpool.query(`select * from account where typegame = ? and account = ?`, [typegamenew, taikhoannew], function(err, edit) {
                if (err) {
                    ...
                    if (edit == "") {
                        ...
                        return res.send(`<script>window.alert("Sửa thất bại vì đã trùng tài khoản"); window.location.href = "/"; </script>`)
                    }
                }
            })
        }
